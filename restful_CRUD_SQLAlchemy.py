...
task_update_args.add_argument("task",type=str,required=False)
task_update_args.add_argument("summary",type=str,required=False)
...
class  ToDo(Resource):
    ...
    def put(self,todo_id):
        ...
        task =ToDoModel.query.filter_by(id=todo_id).first()
        if not task:
            abort(404,message="task doesn't exit, cannot update")
        if args['task']:
            task.task = args['task']
        if args['summary']:
